...
import algorithm.MonteCarlo.evaluators.*;
 ...
 */
public class MonteCarloTreeSearch extends PacmanController {
    ...
    public MOVE getMove(Game game, long timeDue) {
        ...
        if (game.getCurrentMaze().graph[myNodeIndex].numNeighbouringNodes > 2)
            return mcts(game);
        else
     ...
     * @return NEUTRAL if no ghosts found nearby else a move to escape from the
     *         ghost
     */
    ...
    /**
     * Computes the next move for pac-man if it is present in a non-junction node
     * (degree < 3)
     * 
     * @param game current game state
     */
    ...
    /**
     * Computes the next move for pac-man if it is present in a junction node
     * (degree > 2)
     * 
     * @param game current game state
     */
