...
public class UCBTunedSelectionPolicy {
    ...
    public double getUcbValue(MonteCarloTreeNode node) {
        ...
        // Math.sqrt(Math.log(node.getParent().getNumberOfVisits()) /
        // node.getNumberOfVisits());
        return node.getAverageScore() + 1 *
                Math.sqrt(2 * Math.log(node.getParent().getNumberOfVisits()) / node.getNumberOfVisits()
                        * Math.min(0.25, node.getNumberOfVisits()));
    }
}